...
Treat the recovery message as a warning, not a fatal error. Use the exposed Search button.`,
  ...
  makeCase({
    ...
    contract: [
      ...
      'A failed replay returns a divergence report with a suggested selector plus resume.from and resume.planDigest',
      'Goal: repair the script explicitly, then resume from the failing step without rerunning earlier steps',
    ],
    task: 'Plan the safe replay-maintenance workflow after selector drift.',
    outputs: [
      plannedCommand('replay'),
      /\.\/replays\/catalog-checkout\.ad/i,
      /--update.*(?:no-op|does not rewrite|retired)|(?:no-op|does not rewrite|retired).*--update/i,
      /manual(?:ly)? (?:edit|repair)/i,
      /--from\s+(?:<[^>]+>|\d+)/i,
      /--plan-digest\s+(?:<[^>]+>|[a-f0-9]{8,})/i,
    ],
    forbiddenOutputs: [/sed\s+-i/i],
  }),
  makeCase({
