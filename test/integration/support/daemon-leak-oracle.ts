// Daemon leak oracle (#1781 B1, feeds #1431): observes an isolated state dir and
// the liveness of the daemons that owned it, then applies the rules in
// `daemon-leak-model.ts` (which documents them and is where they are tested).
// Daemon-owned child processes are reconstructed by the manual
// `daemon-owned-process-probe.ts` instead — see its header for why that arm
// cannot assert anything from a device-free lane.
...
function captureDaemonLeakSnapshot(options: DaemonLeakOracleOptions): DaemonLeakSnapshot {
...
async function settleDaemonLeakSnapshot(
  ...
  let snapshot = captureDaemonLeakSnapshot(options);
  while (hasDaemonLeaks(snapshot) && Date.now() < deadline) {
    await new Promise((resolve) => setTimeout(resolve, SETTLE_POLL_MS));
    snapshot = captureDaemonLeakSnapshot(options);
  }
  return snapshot;
}
