...
test.each(['wait', 'replay'] as const)(
  'Apple %s captures like is when native selector queries fail',
  async (command) => {
      ...
      appleRunnerProvider: () => ({
        ...
        runCommand: async (_device, command) => {
          ...
          if (command.command === 'querySelector') {
            throw new AppError(
              'COMMAND_FAILED',
              'XCTest recorded a failure while executing querySelector',
    ...
    try {
      ...
      fs.writeFileSync(scriptPath, String.raw`wait "id=\"LANDMARK_A\"" 200` + '\n');
      assertRpcOk(
        await daemon.callCommand(
          command,
          command === 'replay' ? [scriptPath] : ['id="LANDMARK_A"', '200'],
          flags,
        ),
      );
      assert.deepEqual(commands, ['snapshot', 'snapshot']);
    } finally {
      await daemon.close();
    }
  },
