...
- Ref frame: session-scoped authorization namespace containing the accessibility refs that a caller may use for mutation. It is distinct from operational observations used internally for selector matching, settling, freshness, and diagnostics.
- Ref-frame epoch: seeded monotonic version of the current ref frame, exposed once per issuing response as `refsGeneration`. It advances when a frame is published, superseded, or expired; it does not make refs stable across frames.
- Ref-frame activation: session transition that makes a captured namespace current for subsequent mutation admission. Complete activation authorizes its plain-ref namespace; partial activation authorizes only the explicitly issued generation-pinned refs. An explicit snapshot leaf may activate a frame for later predeclared batch/replay steps even when the outer response elides the tree.
- Ref issuance: client- or workflow-visible publication of refs with their ref-frame epoch. Internal freshness, recovery, diagnostic, and cached captures do not issue refs.
- Ref generation pin: optional `~s<n>` suffix on an @ref carrying the ref-frame epoch that issued it. Accepted as input everywhere; daemon tree payloads carry the epoch once at response level rather than per node, MCP may render or forward pinned refs, and durable replay scripts strip pins because epochs are session-specific.
