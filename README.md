...
Let your coding agent check its changes in the running app.

`agent-device` lets coding agents inspect and control apps on iOS, Android, TV, web, macOS, and Linux. Agents can read concise accessibility snapshots, find elements by ref or selector, run device actions, and save evidence for review.
...
`agent-device` uses the inspect-act-check process from Vercel's [agent-browser](https://github.com/vercel-labs/agent-browser) for mobile, TV, and desktop apps. Basic `--platform web` support runs `agent-browser` in the same session and replay system.
...
Use refs only from the latest output. After a command with `--settle`, use the refs in its diff. Take another snapshot only if the diff omits what you need.

Snapshots use the app's accessibility tree. Clear labels, roles, and test IDs make agent runs more reliable. Use screenshots and videos as evidence or when accessibility data is poor. Use refs and selectors for actions and checks when you can.
...
- Inspect the app through accessibility snapshots, refs, selectors, and React Native component trees.
- Use the UI by tapping or pressing elements, filling fields, scrolling, making gestures, waiting, checking state, and handling alerts.
- Find the cause of failures with screenshots, video, logs, traces, network data, performance samples, crash details, and React profiles.
- Save working steps as `.ad` scripts for local use or CI. Export strict Maestro YAML when needed.

See [Commands](https://oss.callstack.com/agent-device/docs/commands) for the commands and evidence each target supports.

![Sketch showing agent-device checking a live app while an agent writes code](./website/docs/public/agentic-development-loop.svg)

## Next steps

- Run the CLI from Cursor, Codex, Claude Code, Windsurf, or another agent terminal. See [AI Agent Setup](https://oss.callstack.com/agent-device/docs/agent-setup) for skills, rules, MCP tools, and setup for each client.
- Clone the repo and run the bundled Expo test app. [Quick Start](https://oss.callstack.com/agent-device/docs/quick-start) covers a guided run with screenshots, replay, and performance data.
- Use [Replay & E2E](https://oss.callstack.com/agent-device/docs/replay-e2e) to repeat tests. Use [Debugging & Profiling](https://oss.callstack.com/agent-device/docs/debugging-profiling) to find bugs.
...
| CI/CD | Checking pull requests and merges with replay scripts and saved files. | Try the [EAS workflow template](https://github.com/callstackincubator/eas-agent-device/blob/main/.eas/workflows/agent-qa-mobile.yml). GitHub Actions template coming soon. |
| Cloud / remote | Linux hosts, hosted devices, and jobs on other machines. | Use [Agent Device Cloud](https://agent-device.dev/cloud), set a remote profile with [Commands](https://oss.callstack.com/agent-device/docs/commands), or [contact Callstack](mailto:hello@callstack.com) for team QA. |
...
Node.js apps can use the typed client or public modules. `agent-device/android-adb` provides the Android ADB provider interface, helpers for logcat, the clipboard, the keyboard, and apps, and reverse port setup.

## FAQ

### What is agent-device?

`agent-device` is a command-line tool that lets coding agents inspect and control apps and save evidence for review. It supports iOS, Android, TV, web, macOS, and Linux.
...
With `agent-device`, an agent reads app state and chooses each command at run time. Teams use Appium, Detox, and Maestro to write and maintain test suites. `agent-device` can save its runs as `.ad` scripts or export them as strict Maestro YAML.

### Can agent-device run in CI?

Yes. Record a run as an `.ad` script, replay it locally or in CI, and save screenshots, logs, and other files for review. See [Replay & E2E](https://oss.callstack.com/agent-device/docs/replay-e2e) or start with the [EAS workflow template](https://github.com/callstackincubator/eas-agent-device/blob/main/.eas/workflows/agent-qa-mobile.yml).
...
`agent-device` is open source under the MIT license. [Contact Callstack](mailto:hello@callstack.com).
