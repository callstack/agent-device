...
export async function stopAndroidNativePerfSession(
  ...
  await pullAndroidNativeArtifact(adb, session, options.files);
  const sizeBytes = await readFileSize(session.outPath, options.files);
...
export async function writeJsonArtifact(
  outPath: string,
  value: unknown,
  files: Pick<AndroidAdbFileHost, 'ensureDirectory' | 'writeBytes'> = requireAndroidAdbHost().files,
): Promise<void> {
  await files.ensureDirectory(path.dirname(outPath));
  await files.writeBytes(outPath, Buffer.from(`${JSON.stringify(value, null, 2)}\n`));
}

export async function readFileSize(
  filePath: string,
  files: Pick<AndroidAdbFileHost, 'stat'> = requireAndroidAdbHost().files,
): Promise<number> {
  try {
    return (await files.stat(filePath)).size;
  } catch (error) {
    throw new AppError(
      'COMMAND_FAILED',
...
async function pullAndroidNativeArtifact(
  adb: AndroidAdbExecutor,
  session: AndroidNativePerfSession,
  files: Pick<AndroidAdbFileHost, 'ensureDirectory'> | undefined,
): Promise<void> {
  await (files ?? requireAndroidAdbHost().files).ensureDirectory(path.dirname(session.outPath));
  try {
