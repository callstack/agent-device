...
test('matches Maestro regex as case-insensitive full patterns', () => {
  expect(matchesMaestroRegex('Item 22\nready', String.raw`item \d{2}.+ready`)).toBe(true);
...
test('extracts only patterns that are exact literals', () => {
  ...
  expect(literalFromMaestroRegex(String.raw`price\.usd`)).toBe('price.usd');
  expect(literalFromMaestroRegex(String.raw`item \d{2}`)).toBeUndefined();
