...
test("metadata diff reports a key only one side carries as absent", () => {
  const expected = resolveExpectedRunnerCacheMetadata(IOS_SIMULATOR);
  const { sdkBuildVersion: _sdkBuildVersion, ...withoutSdkBuildVersion } =
    expected;

  assert.deepEqual(
    diffComparableRunnerCacheMetadata(
      expected,
      withoutSdkBuildVersion as typeof expected,
    ),
...
test("metadata diff is empty for identical metadata", () => {
  ...
  assert.deepEqual(
    diffComparableRunnerCacheMetadata(expected, { ...expected }),
    [],
  );
});
...
test("a timed-out probe leaves the toolchain unavailable instead of a comparable value", () => {
  runCmdSync.mockImplementation((command: string, args: readonly string[]) => {
    if (command === "xcodebuild") {
      throw new AppError(
        "COMMAND_FAILED",
        "xcodebuild timed out after 5000ms",
        {
          timeoutMs: 5_000,
        },
      );
  ...
  assert.deepEqual(unavailableProbes(), [
    { probe: "xcodebuild -version", reason: "probe_error" },
  ]);
});
...
test("a malformed xcodebuild answer is not memoized: the next request re-probes and recovers", () => {
  ...
  assert.equal(metadata.xcodeBuildVersion, "17C52");
  expect(
    runCmdSync.mock.calls
      .slice(probeCallsWhileMalformed)
      .map(([command]) => command),
  ).toEqual(["xcodebuild", "xcrun", "xcrun"]);
});
...
test("only a complete, parsed toolchain fingerprint is memoized", () => {
  ...
  assert.deepEqual(
    [
      second.xcodeVersion,
      second.xcodeBuildVersion,
      second.sdkVersion,
      second.sdkBuildVersion,
    ],
    [
      first.xcodeVersion,
      first.xcodeBuildVersion,
      first.sdkVersion,
      first.sdkBuildVersion,
    ],
  );
});
