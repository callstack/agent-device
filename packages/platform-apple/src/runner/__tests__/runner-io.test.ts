...
import { readRunnerLogTail, tailRunnerLogFile } from '../runner-io.ts';
import { mkdtempForTestSync } from './tmp-dir.ts';

function logFile(prefix = 'runner-log-tail-'): string {
...
test('the tail starts at the offset it was given', async () => {
...
test('a tail whose file disappears stops on its own instead of throwing', async () => {
  ...
  const tail = tailRunnerLogFile({ logPath, offset: 0, onOutput: () => {} });
  fs.rmSync(logPath);
...
test('drain reads what the file gained and then stops following it', async () => {
  const logPath = logFile();
  const chunks: string[] = [];
  const tail = tailRunnerLogFile({ logPath, offset: 0, onOutput: (chunk) => chunks.push(chunk) });
...
test('the log tail an error can quote is the end of the file, bounded', () => {
  ...
  assert.equal(readRunnerLogTail(logPath, 64), `${'x'.repeat(50)}SIGNATURE-TEXT`);
  assert.equal(readRunnerLogTail(logPath, 4_096).endsWith('SIGNATURE-TEXT'), true);
  assert.equal(readRunnerLogTail(undefined, 64), '');
  assert.equal(readRunnerLogTail(path.join(path.dirname(logPath), 'missing.log'), 64), '');
});
