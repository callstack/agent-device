...
function installRecordingHost(calls: AppleToolCall[], mutatePlist = false): void {
  appleRunnerTestHost.update({
    ...
    readApplePlistJson: async (plistPath: string) => JSON.parse(fs.readFileSync(plistPath, 'utf8')),
  });
}
...
test('copies app icon artifacts into synthesized simulator XCTest runner app', async () => {
  await withTempDir('agent-device-runner-icon-', async (root) => {
    ...
    installRecordingHost(calls, true);

    await applyXctestRunnerAppIcon([sourceAppPath, runnerAppPath]);
...
test('finds simulator XCTest runner app from derived data', async () => {
  await withTempDir('agent-device-runner-icon-', async (root) => {
    ...
    appleRunnerTestHost.update({
      runAppleToolCommand: async (): Promise<ExecResult> => ({
        exitCode: 0,
        stdout: '',
        stderr: '',
      }),
      readApplePlistJson: async (plistPath: string) =>
        JSON.parse(fs.readFileSync(plistPath, 'utf8')),
    });

    await applyXctestRunnerAppIconFromDerivedPath(root);
