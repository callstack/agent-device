...
export async function buildRunnerEarlyExitError(params: {
  ...
  // The runner writes its own output file since #2681, so that file - not the exec result, which
  // holds nothing for a file-backed child - is what an early exit can quote. Only a session whose
  // runner still had pipes falls back to the captured streams.
  const output =
    session.readLogTail?.(RUNNER_EARLY_EXIT_LOG_TAIL_BYTES) ||
