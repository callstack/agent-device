...
import { resolveKeyboardTapOcclusion } from '@agent-device/contracts/tap-keyboard-occlusion';
...
// The keyboard band the bridge publishes (#2660). The bridge cannot query the keyboard the way the
// runner does, so what it publishes has to BE the band the shared rule measures over the tree it
// decoded — anything else would move the tap guard's decision without anyone deciding to.

const PORTRAIT_KEYBOARD_TREE = {
  [application]: 'Application',
  [frame]: { X: 0, Y: 0, Width: 402, Height: 874 },
  [children]: [
    {
      [application]: 'UIRemoteKeyboardWindow',
      [automationType]: 4,
      [frame]: { X: 0, Y: 583, Width: 402, Height: 233 },
      [children]: [
        {
          [automationType]: 19,
          [frame]: { X: 0, Y: 583, Width: 402, Height: 233 },
          [children]: [0, 1, 2, 3].map((column) => ({
            [automationType]: 20,
            [frame]: { X: column * 100.5, Y: 600, Width: 100.5, Height: 60 },
            [children]: [],
          })),
        },
      ],
...
test('a portrait keyboard-up capture publishes the band the tree rule measures', () => {
  const decoded = decodeSnapshotBridgeTree(PORTRAIT_KEYBOARD_TREE, { truncated: false }, limits);

  // Reported key plane bottoms out at 816 of an 874 pt viewport, and the band still owns the strip
  // below it: the same frame the tap guard computed from this tree before the fact existed.
  assert.deepEqual(decoded.keyboard, {
    kind: 'visible',
    frame: { x: 0, y: 583, width: 402, height: 291 },
  });

  const point = { x: 201, y: 832 };
  const measuredByTheRule = resolveKeyboardTapOcclusion({
    nodes: decoded.nodes,
    viewport: decoded.viewport.kind === 'missing' ? null : decoded.viewport.rect,
    point,
  });
  assert.equal(measuredByTheRule.kind, 'occluded');
  assert.deepEqual(
    decoded.keyboard.kind === 'visible' ? decoded.keyboard.frame : null,
    measuredByTheRule.surface.frame,
    'the published band must be the band the rule measures',
  );
});

test('a tree with no keyboard window publishes a proven absence', () => {
  const decoded = decodeSnapshotBridgeTree(
    {
      [application]: 'Application',
      [frame]: { X: 0, Y: 0, Width: 402, Height: 874 },
      [children]: [
        {
          [application]: 'UIWindow',
          [automationType]: 4,
          [frame]: { X: 0, Y: 0, Width: 402, Height: 874 },
          [children]: [],
        },
  ...
  assert.deepEqual(decoded.keyboard, { kind: 'absent' });
});

test('a keyboard this tree cannot measure is published as unmeasurable, not as absent', () => {
  // An iPad floating keyboard reports a band the rule refuses to extend to the bottom edge. The
  // screen is not clear of a keyboard, and the guard has to be able to say it abstained.
  const decoded = decodeSnapshotBridgeTree(
    {
      [application]: 'Application',
      [frame]: { X: 0, Y: 0, Width: 402, Height: 874 },
      [children]: [
        {
          [application]: 'UIRemoteKeyboardWindow',
          [automationType]: 4,
          [frame]: { X: 21, Y: 480, Width: 360, Height: 200 },
          [children]: [
            {
              [automationType]: 19,
              [frame]: { X: 21, Y: 480, Width: 360, Height: 200 },
              [children]: [0, 1, 2].map((column) => ({
                [automationType]: 20,
                [frame]: { X: 21 + column * 120, Y: 500, Width: 120, Height: 60 },
                [children]: [],
              })),
            },
          ],
  ...
  assert.deepEqual(decoded.keyboard, {
    kind: 'unmeasurable',
    reason: 'tree-keyboard-band-unmeasurable',
  });
});
