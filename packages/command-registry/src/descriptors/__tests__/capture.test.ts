import { CAPTURE_COMMAND_DESCRIPTORS } from '../capture.ts';
...
test('capture declares exactly its own commands', () => {
  expect(CAPTURE_COMMAND_DESCRIPTORS.map((descriptor) => descriptor.name).sort()).toEqual([
    'alert',
    'diff',
    'screenshot',
    'settings',
