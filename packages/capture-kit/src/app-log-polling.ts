...
/**
 * Provider-supplied app-log source. A reader may or may not honour `signal`; the poller settles a
 * bounded read on its own timeout either way, so abort never proves the provider request ended.
 */
...
async function boundedRead(
  ...
  const read = settleOnAbort(
    input.reader.readLogs(input.appBundleId, READ_LINE_LIMIT, controller.signal),
    controller.signal,
