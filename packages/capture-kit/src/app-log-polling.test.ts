...
describe('app-log poller', () => {
  test('deduplicates the persisted tail, echoes backend identity, and stops before disposing', async () => {
    ...
    const readLogs = vi.fn(
      async (_appBundleId: string, _lineLimit: number, _signal?: AbortSignal) =>
        'old line\nshared\nnew line\n',
    );
    ...
    expect(readLogs.mock.lastCall?.[2]).toBeInstanceOf(AbortSignal);
    expect((readLogs.mock.lastCall?.[2] as AbortSignal | undefined)?.aborted).toBe(true);
  });
  ...
  test('fails a bounded read whose reader ignores its signal, then disposes', async () => {
    const sleeps = createDeferredSleeps();
    let readerDisposed = false;
    const readLogs = vi.fn(async (_appId: string, _limit: number, _signal?: AbortSignal) => {
  ...
  test('settles a signal-honoring reader whose request rejects on abort', async () => {
    const sleeps = createDeferredSleeps();
    let readerDisposed = false;
    const readLogs = vi.fn(
      (_appId: string, _limit: number, signal?: AbortSignal) =>
