...
- On hosted WebDriver sessions (BrowserStack, AWS Device Farm), `fill` waits until the field it tapped actually holds text entry focus before sending its keys, and reports what it observed as `textEntryReadiness`: `focused-element` (the focused element contains the tapped point — the strongest signal, and the only one that identifies *which* field took focus), `keyboard-shown` (the driver has no active-element route, but the keyboard rose from hidden after the tap), or `settled-unknown` (the driver implements neither route, so there was nothing to wait for).
