...
- Face ID and Touch ID controls are iOS simulator-only. They post the `com.apple.BiometricKit_Sim` notifications the Simulator.app Features menu posts (`xcrun simctl spawn <device> notifyutil`), so they work on every Xcode; `simctl` has no `biometric` subcommand.
