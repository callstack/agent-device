...
- When the Apple runner records (`--fps` sets its frame rate), each frame must be captured within one frame interval. A frame the runner cannot capture in time, or that falls while earlier main-thread work is still draining, is dropped instead of queued, so a busy app can yield fewer frames than `--fps` requests.
