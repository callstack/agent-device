...
- `screenshot --fullscreen` on macOS applies only to app sessions. The `desktop` and `menubar` surfaces always capture the main display through the macOS helper, so an explicit `--fullscreen` on either names a frame the capture cannot vary; it is refused with `INVALID_ARGS` before any capture runs, rather than accepted and ignored.
