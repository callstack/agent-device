...
- A client only reuses a running daemon when the daemon's version matches its own and, for a client running from a source checkout, when the daemon's code fingerprint matches too; otherwise the daemon is restarted. A published version is never served twice with different contents, so an installed client has nothing further to compare — its version already fixes the bytes it runs, and demanding a fingerprint instead would make two installs of one version replace each other's daemon and drop the live session on it. The residual limit: an install whose files were replaced under its own version (a hand-edited `node_modules/agent-device`, or a registry that served one version twice) is not detected. The fingerprint compares file sizes and timestamps, never contents, and two installs of identical bytes never agree on those anyway.
