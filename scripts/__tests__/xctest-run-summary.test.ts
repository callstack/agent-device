// A whole-bundle XCTest lane's reporter is also its liveness check, and both halves only
// ever execute on a macOS runner — so they are proven here instead.
...
const TITLE = 'iOS runner full XCTest suite';
...
describe('the liveness check', () => {
  ...
  test('a summary missing the count entirely is treated as no tests, not as unknown', () => {
    expect(livenessFailure({}, 153)).not.toBeNull();
  });

  test('a run that executed exactly the source-derived reach passes, red or green', () => {
    expect(
      livenessFailure({ totalTestCount: 153, result: 'Failed', failedTests: 9 }, 153),
    ).toBeNull();
    expect(livenessFailure({ totalTestCount: 1, result: 'Passed' }, 1)).toBeNull();
  });

  test('a run that executed fewer or more tests than the source reaches fails', () => {
    // The quieter failure "at least one ran" never sees: a `#if` guard that compiles a file
    // of tests out of the lane's platform, or a flag that matched more than it names.
    const fewer = livenessFailure({ totalTestCount: 120, result: 'Passed' }, 133);
    expect(fewer).toContain('executed 120 test(s) but the source says it reaches 133');
    expect(fewer).toContain('check:xctest-selection');
    expect(livenessFailure({ totalTestCount: 134, result: 'Passed' }, 133)).not.toBeNull();
  });

  test('a lane the source says reaches nothing can never be credible', () => {
    expect(livenessFailure({ totalTestCount: 0 }, 0)).toContain('derives no reachable tests');
    expect(livenessFailure({ totalTestCount: 5 }, 0)).toContain('derives no reachable tests');
  });
});
...
describe('the job summary', () => {
  ...
  test('leads with the headline a reader needs', () => {
    const rendered = renderSummary(green, TITLE, 153);
    expect(rendered).toContain('- result: **Passed**');
    expect(rendered).toContain(
      '- executed: **153** of the 153 the source reaches on this lane (passed 153, failed 0, skipped 0',
    );
  ...
  test('reports an unknown duration rather than NaN when the times are absent', () => {
    expect(renderSummary({ totalTestCount: 1 }, TITLE, 1)).toContain('- duration: unknown');
  });

  test('lists failures and names each one', () => {
    const rendered = renderSummary(
      { ...green, result: 'Failed', testFailures: failures(3) },
      TITLE,
      153,
    );
  ...
  test('caps the list, and says how many it dropped', () => {
    ...
    // possible trade.
    const rendered = renderSummary(
      { ...green, result: 'Failed', testFailures: failures(MAX_LISTED_FAILURES + 7) },
      TITLE,
      153,
    );
  ...
  test('truncates one enormous failure message instead of letting it dominate', () => {
    const rendered = renderSummary(
      { ...green, result: 'Failed', testFailures: failures(1, 'x'.repeat(5000)) },
      TITLE,
      153,
    );
