...
function renderSpikeMarkdown(report: SpikeReport): string {
  const lines = [
    ...
    `- Host client: **${report.guestMechanism.client}**; no idb_companion, gRPC, or Python is involved, and nothing here ships in the npm package.`,
    '',
    ...
    '|---|---|---|---|---|---|',
    `| guest-simulator-framework-bridge | in-Simulator SimulatorFrameworkBridge reader over a UNIX socket, driven from Node | ${surfaceStatus(report, 'guest-simulator-framework-bridge', 'app')} | ${surfaceStatus(report, 'guest-simulator-framework-bridge', 'system')} | one private guest per session, typed failures, respawn on drop | XCTest view-hierarchy tree; hittability and placeholder are not fetched by the single-fetch traversal |`,
    ...
    '## Decision rationale',
    '',
    ...(report.decisionReasons.length === 0
      ? ['- Every hard gate passed.']
      : report.decisionReasons.map((reason) => `- ${reason}`)),
    '',
    '## Stretch findings',
    '',
    ...(report.stretchFindings.length === 0
      ? ['- None.']
