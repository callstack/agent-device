...
export function createAdapterOptions(config: SpikeConfig): AdapterOptions {
  return {
    ...
    ...(config.guestBridge ? { guestBridge: config.guestBridge } : {}),
...
async function collectWarmSamples(
  ...
  let appPid = await admitReadyCell(context, admission);
  if (appPid === undefined)
...
async function collectNonWarmSamples(
  ...
  for (let index = 0; index < config.samples; index += 1) {
    ...
    appPid = await admitSuccessfulSample(context, admission, opened, appPid);
    const request = makeRequest(
      config,
      adapter.candidate,
      admission.state,
    ...
    cleanupSuccessfulSample(context, admission);
  }
}
