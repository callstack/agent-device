...
export function createPrivateCoreSimulatorAxAdapter(options: AdapterOptions): AcquisitionAdapter {
  const limits = options.limits ?? DEFAULT_SPIKE_LIMITS;
  if (!options.privateTool || !fs.existsSync(options.privateTool)) {
    return unsupportedAdapter('private-coresimulator-ax', 'private-tool-unavailable', limits);
  }
  return framedAdapter('private-coresimulator-ax', options.privateTool, limits);
}
      ...
      return {
        responses: requests.map((request) =>
          failureResponse(
            ...
            {
              requestBytes: Buffer.byteLength(JSON.stringify({ ...request, limits })) + 1,
            },
          ),
        ),
        stderr: '',
