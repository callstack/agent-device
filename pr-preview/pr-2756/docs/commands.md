...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-2756/docs/configuration.md).
...
- Use [Remote Proxy](/agent-device/pr-preview/pr-2756/docs/remote-proxy.md) when you need to run `agent-device proxy` on a Mac with simulator/device access and drive it from another machine through cloudflared, ngrok, or another HTTP tunnel.
- Use [BrowserStack](/agent-device/pr-preview/pr-2756/docs/browserstack.md) or [AWS Device Farm](/agent-device/pr-preview/pr-2756/docs/aws-device-farm.md) when a CI agent needs a hosted device session without interactive login.
...
See [remote takeover and host administration](/agent-device/pr-preview/pr-2756/docs/remote-proxy.md#human-takeover) for the VM-side API.
...
- `--actions` names the custom accessibility affordances an element merged away (iOS
  ...
  [Snapshots](/agent-device/pr-preview/pr-2756/docs/snapshots.md) for the full constraints.
...
[Migrating Gestures](/agent-device/pr-preview/pr-2756/docs/migrating-gestures.md) if you have scripts or recordings that still pass one.
...
- `replay -u`/`--update` no longer rewrites the script (retired — see [Replay & E2E](/agent-device/pr-preview/pr-2756/docs/replay-e2e.md)); it is a no-op kept for compatibility, since every divergence already carries the same ranked suggestions.
...
See [Replay & E2E](/agent-device/pr-preview/pr-2756/docs/replay-e2e.md) for recording, Maestro compatibility, and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-2756/docs/batching.md) for payload format, response shape, and usage guidelines.
...
- `record contact-sheet <video.mp4> [--out <sheet.png>] [--no-diff-overlay]` turns a recording you already exported into one PNG: the frames where the screen visibly changed, laid out in a grid and each labeled with its elapsed time (`HH:MM:SS.mmm`) on the clip timeline. It is how an agent reads a recording it cannot play. It reads the file you pass — no session, no device, no daemon — so it can rebuild an old take and the sheet can only describe screens that export contains. Any backend that exports MP4 works (Apple, Android, HarmonyOS, Limrun); a WebM recording from the web backend is refused with `details.reason: contact_sheet_container_unsupported`. The default output is `<recording>.contact-sheet.png`, and `--out` is refused when it points back at the recording itself, because a sheet is derived from a take and never replaces it. Each cell is boxed where its pixels moved since the cell before it — a red border over a light wash — so the change is findable at a glance; a change that repaints most of the screen is left unboxed, because a box over the whole frame points at nothing. Pass `--no-diff-overlay` for unmarked frames. `--json` reports each cell's time and changed-pixel share alongside `sampledFrameCount`, `decodedFrameCount`, `skippedSampleCount`, and `diffOverlay`.
...
- Historical lookup requires `--provider-session <id>` and `--provider <name>`. BrowserStack uses `BROWSERSTACK_USERNAME` and `BROWSERSTACK_ACCESS_KEY`. AWS Device Farm uses the AWS CLI credential chain and infers the region from the session ARN when possible. See [BrowserStack](/agent-device/pr-preview/pr-2756/docs/browserstack.md) and [AWS Device Farm](/agent-device/pr-preview/pr-2756/docs/aws-device-farm.md) for CI credential setup.
