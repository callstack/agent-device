...
iOS text entry is witnessed rather than assumed. `fill` taps the target, waits for the instance to name the element that owns text entry, and only then types, so an app that exposes fields without ever publishing a globally focused element - Flutter forms are the common case - still fills, and the result reports `textEntryReadiness: "focused-element"`. A tap that nothing answers fails with reason `text_entry_focus_not_observed` instead of typing into an unknown field. `type` stays the deliberate route that writes into whichever field already holds focus.
