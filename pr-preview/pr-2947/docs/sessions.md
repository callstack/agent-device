...
- On iOS, `appstate` answers about the session app: its name from the session record and, when a
  ...
  the foreground during a command is the [`targetActivation` disclosure](/agent-device/pr-preview/pr-2947/docs/commands.md#foreground-repairs-on-ios).
- For remote `connect --remote-config` sessions, see [Commands](/agent-device/pr-preview/pr-2947/docs/commands.md#remote-metro-workflow).
...
For replay scripts and deterministic E2E guidance, see [Replay & E2E](/agent-device/pr-preview/pr-2947/docs/replay-e2e.md).
