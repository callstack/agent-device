...
Make an iOS simulator or device [available to agent-device](/agent-device/pr-preview/pr-2665/docs/agent-setup.md), then configure an AI SDK model. String model IDs use AI Gateway by default:
...
See the [Node.js API](/agent-device/pr-preview/pr-2665/docs/client-api.md) when the host application needs deterministic setup or other direct device control outside the agent loop. See the AI SDK reference for [`ToolLoopAgent`](https://ai-sdk.dev/docs/reference/ai-sdk-core/tool-loop-agent).
