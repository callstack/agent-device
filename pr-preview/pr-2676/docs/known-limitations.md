...
`agent-device` reaches the Android clipboard through `adb shell cmd clipboard`. That command works only on a build whose clipboard service implements a shell command, and AOSP's `ClipboardService` does not: on Android 16 (API 36) every `cmd clipboard get text` and `cmd clipboard set text <text>` call is answered by the framework's default `Binder.handleShellCommand` — `No shell command implementation.` on stderr, exit status **0** — so the clipboard is never touched even though the call reports success.
...
**Workaround:** verify a copy flow from the app side — trigger the app's copy action, paste into a focused text field, and read that field back with `snapshot`. It proves the app's own clipboard write, which an adb-side read never did. On a build that does implement `cmd clipboard`, note that Android 10+ restricts clipboard reads to the app with input focus or the current input method service (adb is neither), so an adb-side read can still come back blank there.
