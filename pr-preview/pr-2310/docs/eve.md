...
Run the tool in Eve's app runtime when it needs local access to simulators, emulators, platform tooling, and daemon state. If Eve is hosted separately from the devices, connect through an [agent-device remote proxy](/agent-device/pr-preview/pr-2310/docs/remote-proxy.md) instead.
...
See [Node.js API](/agent-device/pr-preview/pr-2310/docs/client-api.md) for the complete client surface.
