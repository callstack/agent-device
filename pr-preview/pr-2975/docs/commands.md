...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-2975/docs/configuration.md).
...
- Use [Remote Proxy](/agent-device/pr-preview/pr-2975/docs/remote-proxy.md) when you need to run `agent-device proxy` on a Mac with simulator/device access and drive it from another machine through cloudflared, ngrok, or another HTTP tunnel.
- Use [BrowserStack](/agent-device/pr-preview/pr-2975/docs/browserstack.md) or [AWS Device Farm](/agent-device/pr-preview/pr-2975/docs/aws-device-farm.md) when a CI agent needs a hosted device session without interactive login.
...
See [remote takeover and host administration](/agent-device/pr-preview/pr-2975/docs/remote-proxy.md#human-takeover) for the VM-side API.
...
- `--actions` names the custom accessibility affordances an element merged away (iOS
  ...
  [Snapshots](/agent-device/pr-preview/pr-2975/docs/snapshots.md) for the full constraints.
...
[Migrating Gestures](/agent-device/pr-preview/pr-2975/docs/migrating-gestures.md) if you have scripts or recordings that still pass one.
...
- `replay -u`/`--update` no longer rewrites the script (retired — see [Replay & E2E](/agent-device/pr-preview/pr-2975/docs/replay-e2e.md)); it is a no-op kept for compatibility, since every divergence already carries the same ranked suggestions.
...
See [Replay & E2E](/agent-device/pr-preview/pr-2975/docs/replay-e2e.md) for recording, Maestro compatibility, and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-2975/docs/batching.md) for payload format, response shape, and usage guidelines.
...
- Historical lookup requires `--provider-session <id>` and `--provider <name>`. BrowserStack uses `BROWSERSTACK_USERNAME` and `BROWSERSTACK_ACCESS_KEY`. AWS Device Farm uses the AWS CLI credential chain and infers the region from the session ARN when possible. See [BrowserStack](/agent-device/pr-preview/pr-2975/docs/browserstack.md) and [AWS Device Farm](/agent-device/pr-preview/pr-2975/docs/aws-device-farm.md) for CI credential setup.
