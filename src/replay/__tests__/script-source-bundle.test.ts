...
test('a native .ad bundle carries the entry script resolved against the caller cwd', () => {
  ...
  const bundle = buildReplayScriptSourceBundle({ inputPath: './flow.ad', cwd: root });

  expect(bundle.entry).toBe(path.join(root, 'flow.ad'));
  expect(bundle.files).toEqual({ [path.join(root, 'flow.ad')]: script });
});

test('a Maestro bundle carries the entry flow and its transitive runFlow includes', () => {
...
// round-trip.
test('a missing entry script fails as INVALID_ARGS naming the path as typed', () => {
  const root = mkdtempForTestSync('agent-device-bundle-missing-');

  expect(() => buildReplayScriptSourceBundle({ inputPath: './flows/login.ad', cwd: root })).toThrow(
...
test('an oversized bundle is refused with the limit named', () => {
  ...
  expect(() => buildReplayScriptSourceBundle({ inputPath: './flow.ad', cwd: root })).toThrow(
    expect.objectContaining({
      code: 'INVALID_ARGS',
