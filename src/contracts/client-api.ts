...
// it. It used to live in `client/client-types.ts`, which made 28 of the layering gate's 30
// remaining type-only inversions a single edge: `commands` declaring itself in terms of the client
// it is a peer of. R2 forbids the reverse import, so the shape has to sit below both.
//
// `client/client-types.ts` re-exports all of this, so the published API is byte-identical; it now
// holds only the `AgentDeviceClient` facade that binds these shapes to methods.
//
// The contents below are a verbatim relocation — no declaration was edited in the move.
