...
test('reads elapsed device uptime as the clock that timestamps recorded frames', async () => {
  const budgets: Record<string, number | undefined> = {};
  const signal = new AbortController().signal;
  await withAndroidAdbProvider(
    {
      exec: async (args: string[], options?: { timeoutMs?: number; signal?: AbortSignal }) => {
        const command = args[1] ?? '';
        budgets[command] = options?.timeoutMs;
        if (command === 'cat /proc/uptime') {
          expect(options?.signal).toBe(signal);
          return result('1234.56 3456.78\n');
        }
        if (command.startsWith('test -e')) return result();
        return command.startsWith('stat -c') ? result('42\n') : result('', 'gone', 1);
      },
    },
    { serial: android.id },
    async () => {
      const transport = await createAndroidScreenRecordingTransport(android);
      await expect(transport.elapsedUptimeMs(signal)).resolves.toBe(1_234_560);
      await expect(transport.size('/sdcard/capture.mp4')).resolves.toBe(42);
    },
  );
  // The optional clock read precedes recording work, so it may not spend the window that work needs.
  expect(budgets['cat /proc/uptime']).toBeLessThan(
    budgets["stat -c %s '/sdcard/capture.mp4'"] as number,
  );
});

test('leaves the recorder clock unanswered when uptime is unreadable or unparseable', async () => {
  for (const reading of [
    result('', 'No such file or directory', 1),
    result('not-a-number 0.00\n'),
    result('\n'),
  ]) {
    await withAndroidAdbProvider(
      { exec: async () => reading },
      ...
      async () => {
        const transport = await createAndroidScreenRecordingTransport(android);
        await expect(transport.elapsedUptimeMs()).resolves.toBeUndefined();
      },
    );
  }
});
