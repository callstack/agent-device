...
export const FACADE_BUDGETS: Readonly<Record<string, number>> = Object.freeze({
  ...
  'packages/contracts/src/facades/platform.ts': 54,
  ...
  'packages/contracts/src/platform-runtime-unavailable.ts': 26,
  ...
  // Added by Wave 6 R55/R56/R57: the clipboard, app-switcher and app-event facets, plus the local
  // interaction set Android and Linux used to hold a byte-identical copy of each. The set is its
  // own module rather than part of the interactor catalog so the catalog's closure stays
  // leaf-thin -- every module the set pulls in is one its two consumers already evaluate.
  ...
  'packages/contracts/src/local-interactor-operation-set.ts': 24,
  'packages/contracts/src/home-runtime.ts': 4,
  'packages/contracts/src/interactor-operation-catalog.ts': 12,
...
export const HUB_BUDGETS: Readonly<Record<string, number>> = Object.freeze({
  ...
  'src/cli.ts': 365,
  'src/platform-runtime.ts': 42,
  'src/core/dispatch.ts': 82,
