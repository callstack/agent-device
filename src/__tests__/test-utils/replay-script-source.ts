 ...
 * (#1802) so a test never hand-rolls a shape the CLI would not actually send. Native loading is
 * synchronous because an `.ad` script has no include grammar and needs no engine; use
...
export function replayScriptSourceBundleFor(filePath: string): ReplayScriptSourceBundle {
  const bundle = loadReplayScriptSourceBundle({ inputPath: filePath, cwd: process.cwd() });
  if (bundle instanceof Promise) {
    throw new Error('native replay source bundle unexpectedly loaded asynchronously');
  }
  return bundle;
}
