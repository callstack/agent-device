...
import { withAppleRunnerProvider, type RunnerCommand } from '@agent-device/platform-apple/runner';
...
import { resolveAppleBackRunnerCommand } from '@agent-device/platform-apple';
...
const mockRunAppleRunnerCommand = vi.fn();

function withoutRunnerCommandId(command: RunnerCommand): Omit<RunnerCommand, 'commandId'> {
  const { commandId: _commandId, ...withoutId } = command;
  return withoutId;
}

async function withRunner<T>(operation: () => Promise<T>, device: DeviceInfo = iosSimulator) {
...
test('resolveAppleBackRunnerCommand defaults plain back to in-app navigation', async () => {
  assert.equal(await resolveAppleBackRunnerCommand(), 'backInApp');
});

test('resolveAppleBackRunnerCommand maps explicit back modes to runner commands', async () => {
  assert.equal(await resolveAppleBackRunnerCommand('in-app'), 'backInApp');
  assert.equal(await resolveAppleBackRunnerCommand('system'), 'backSystem');
});
...
test('ios scroll sends a single fused scroll command and reports planned pixels', async () => {
  ...
  mockRunAppleRunnerCommand.mockImplementation(async (_device, command) => {
    commands.push(withoutRunnerCommandId(command));
  ...
  const result = await withRunner(() => interactor.scroll('down', { pixels: 120 }));
...
test('ios amount-based scroll recomputes pixels from the runner reference frame', async () => {
  ...
  const interactor = await getInteractor(iosSimulator, { appBundleId: 'com.example.app' });
  const result = await withRunner(() => interactor.scroll('down', { amount: 0.5 }));
...
test('tvOS scroll sends only a remotePress command (behavior unchanged)', async () => {
  ...
  await withRunner(() => interactor.scroll('down'), tvOsSimulator);
...
test('tvOS back navigates focus via the remote Menu button, not a coordinate tap', async () => {
  ...
  await withRunner(() => interactor.back(), tvOsSimulator);
...
test('iOS back keeps the in-app navigation path, not the tvOS remote', async () => {
  ...
  await withRunner(() => interactor.back());
...
test('ios scroll rejects non-positive amount before sending any runner command', async () => {
  ...
  await assert.rejects(
    () => withRunner(() => interactor.scroll('down', { amount: 0 })),
    (error: unknown) =>
