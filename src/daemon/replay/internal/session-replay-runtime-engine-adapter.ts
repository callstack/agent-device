 ...
 * of `native-command.ts`, which now only constructs a `ReplayStepContext` and calls
 * `createAdReplayStepRuntime`. See that file's `runReplayCommand` for the request-level
 ...
 * the engine actually sees; `readLastResponse` lets `runReplayCommand`
...
export function createAdReplayStepRuntime(params: {
   ...
   * `runReplayCommand`'s catch block reports, so a mid-loop throw still names
