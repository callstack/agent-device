...
Text entry:
  ...
  Only dismiss the keyboard when hiding it is the actual goal. To hide the keyboard, use keyboard dismiss. It taps safe controls like Done when available, falls back to a snapshot-derived safe-area tap outside the keyboard when no dismiss key is exposed, and verifies the keyboard closed. The response discloses which mechanism worked (dismissKey or safeAreaTap); do not treat the two as equally reliable — a safe-area tap depends on the app resigning focus on background taps and is not guaranteed.
