...
Text entry:
  ...
  Only dismiss the keyboard when hiding it is the actual goal. To hide the keyboard, use keyboard dismiss. It taps safe controls like Done when available and verifies the keyboard closed. On iOS, if keyboard dismiss returns UNSUPPORTED_OPERATION and you must hide the keyboard, first run snapshot -i. Then press a visible static text or heading that has no app action, and verify the keyboard disappears. Do not use buttons, links, switches, back, home, or coordinate taps solely to hide the keyboard. If there is no clearly inert text or heading, report that keyboard dismissal is unavailable.
