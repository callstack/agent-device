...
Text entry:
  ...
  Only dismiss the keyboard when hiding it is the actual goal. To hide the keyboard, use keyboard dismiss. It taps safe controls like Done when available and verifies the keyboard closed. If it reports UNSUPPORTED_OPERATION on iOS and no next control is intended, inspect snapshot -i and press a visible static text or heading that is clearly inert; verify the keyboard disappeared from the settled diff or a snapshot. Use an app button such as Done only when its app action is intended. Never use a button, link, switch, back, home, or coordinates solely to blur the iOS keyboard. If no clearly inert target exists, report that keyboard dismissal is unavailable.
