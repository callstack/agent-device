...
// --- policy-backed commands -----------------------------------------------------------------
// `suggest` names the next element the way `find` does, and `act` is a loop over the verbs above,
// so both belong to this family's vocabulary. They are declared here rather than in their own
// module because the CLI surface is part of the closure every invocation evaluates at startup,
// and a new module there would grow it; the policy runtime they call lives under commands/policy
// and is loaded on demand by the client.
...
const actCommandFacet = defineCommandFacet({
  ...
  cliOutputFormatter: interactionCliOutputFormatters.act,
});
